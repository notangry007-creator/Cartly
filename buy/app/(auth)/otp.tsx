  ...
  function resend() {
    ...
    if (!result.success) {
      if (result.error === 'rate_limited') {
        setError(`Too many requests. Wait ${result.retryAfterSeconds}s.`);
        return;
      }
    }
    setDigits(['', '', '', '', '', '']);
    ...
    <View style={[s.container, { paddingTop: insets.top, paddingBottom: insets.bottom + SPACING.lg }]}>
      ...
      <View style={s.content}>
        ...
        <View style={s.otpRow}>
          {digits.map((d, i) => (
            <RNTextInput
              ...
              maxLength={1}
              style={[s.digit, error ? s.digitErr : null]}
        ...
        <View style={s.resendRow}>
          ...
          {resendDisabled
            ? <Text variant="bodySmall" style={s.cd}>Resend in {countdown}s</Text>
            : (
              <TouchableOpacity onPress={resend}>
                <Text variant="bodySmall" style={s.rl}>Resend OTP</Text>
              </TouchableOpacity>
            )
          }
        </View>
...
const s = StyleSheet.create({
  ...
  demoBox: {
    ...
    backgroundColor: '#FFF8E1', alignSelf: 'stretch',
  },
  demoTxt: { color: '#666', flex: 1 },
  otpRow: { flexDirection: 'row', gap: SPACING.sm, marginVertical: SPACING.sm },
  digit: {
    width: 46, height: 54, borderWidth: 2, borderColor: '#e0e0e0',
    borderRadius: RADIUS.md, textAlign: 'center', fontSize: 22,
    fontWeight: '700', color: '#222', backgroundColor: '#fafafa',
  },
