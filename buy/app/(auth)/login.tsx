...
export default function LoginScreen() {
  ...
  const { control, handleSubmit, formState: { errors, isSubmitting }, setError } = useForm<F>({
  ...
  const onSubmit = async (data: F) => {
    ...
    if (!result.success) {
      if (result.error === 'rate_limited') {
        setError('phone', {
          message: `Too many requests. Please wait ${result.retryAfterSeconds}s before trying again.`,
        });
        return;
      }
    ...
    // Navigate to OTP screen — OTP is NOT passed as a param for security
    // The OTP is stored server-side (in-memory for demo) and verified there
  ...
  return (
    <KeyboardAvoidingView style={{ flex: 1 }} behavior={Platform.OS === 'ios' ? 'padding' : 'height'}>
      <ScrollView
        contentContainerStyle={[s.container, { paddingTop: insets.top + SPACING.xl, paddingBottom: insets.bottom + SPACING.lg }]}
