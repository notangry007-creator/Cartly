...
const STATUS_COLORS: Record<OrderStatus, string> = {
  ...
  delivered: '#2E7D32',
  cancelled: '#B71C1C',
  return_requested: '#F57F17',
  return_approved: '#558B2F',
  return_picked: '#00695C',
  refunded: '#37474F',
};
...
export default function OrdersScreen() {
  ...
  if (!user) {
    return (
      <View style={[s.container, { paddingTop: insets.top }]}>
        <View style={s.header}>
          <Text variant="headlineSmall" style={s.headerTitle}>Orders</Text>
        </View>
        <View style={s.empty}>
          <Ionicons name="receipt-outline" size={64} color="#ccc" />
          <Text variant="titleMedium" style={s.emptyTitle}>Please login</Text>
  ...
  return (
    <View style={[s.container, { paddingTop: insets.top }]}>
      <View style={s.header}>
        <Text variant="headlineSmall" style={s.headerTitle}>My Orders</Text>
      </View>
      ...
      <View style={s.filters}>
        {(['all', 'active', 'delivered', 'cancelled'] as Filter[]).map(f => (
          <Chip
            ...
            textStyle={filter === f ? s.chipTxtA : s.chipTxt}
            compact
      ...
      <FlatList
        ...
        ListEmptyComponent={
          <View style={s.empty}>
            <Ionicons name="receipt-outline" size={64} color="#ccc" />
            <Text variant="titleMedium" style={s.emptyTitle}>No orders yet</Text>
            <Button mode="contained" onPress={() => router.push('/(tabs)/home')}>Start Shopping</Button>
          </View>
        }
        renderItem={({ item }) => (
          <TouchableOpacity
            ...
            activeOpacity={0.8}
            accessibilityRole="button"
            accessibilityLabel={`Order ${item.id.slice(-8).toUpperCase()}, ${STATUS_LABELS[item.status]}, total ${formatNPR(item.total)}`}
          >
            <Surface style={s.card} elevation={1}>
              ...
              <View style={s.cardBody}>
                ...
                <View style={s.itemInfo}>
                  ...
                  {item.items.length > 1 && (
                    <Text variant="labelSmall" style={s.more}>
                      +{item.items.length - 1} more
                    </Text>
                  )}
                  <Text variant="labelSmall" style={s.date}>
                    {formatDate(item.createdAt)}
                  </Text>
                </View>
                <View style={s.amount}>
                  <Text variant="titleSmall" style={s.total}>{formatNPR(item.total)}</Text>
                  <Text variant="labelSmall" style={s.method}>
                ...
                <Text variant="labelSmall" style={s.eta}>
                  {item.status === 'delivered'
                    ? 'Delivered ' + formatDate(
                        item.timeline.find(t => t.status === 'delivered')?.timestamp ?? item.expectedDelivery
                      )
