...
export default function NewAddressScreen() {
  ...
  const { control, handleSubmit, formState:{errors} } = useForm<F>({ resolver:zodResolver(schema), defaultValues:{label:'Home',province:'Bagmati',district:'Kathmandu',municipality:'Kathmandu',ward:1,landmark:'',street:''} });
  const onSubmit = async (data: F) => {
    if(!user) return;
    await createAddr({ userId:user.id, label:data.label, province:data.province, district:data.district, municipality:data.municipality, ward:data.ward, landmark:data.landmark, street:data.street, latitude:27.7172, longitude:85.3240, isPickupPointFallback:isPickup, isDefault });
    router.back();
  };
  const FI = ({name, label, keyboardType, required}: {name:keyof F;label:string;keyboardType?:any;required?:boolean}) => (
    <Controller control={control} name={name} render={({field:{onChange,value}})=><TextInput label={label+(required?' *':'')} value={String(value??'')} onChangeText={onChange} mode="outlined" keyboardType={keyboardType} error={!!errors[name]}/>}/>
  );
  return (
    <View style={[s.container,{paddingTop:insets.top}]}>
      ...
      <ScrollView style={s.scroll} showsVerticalScrollIndicator={false}>
        <View style={s.content}>
          ...
          <View style={s.switchRow}><View><Text variant="bodyMedium" style={s.switchLabel}>Set as Default</Text><Text variant="labelSmall" style={s.switchSub}>Use this address by default</Text></View><Switch value={isDefault} onValueChange={setIsDefault} color={theme.colors.primary}/></View>
          <View style={s.switchRow}><View><Text variant="bodyMedium" style={s.switchLabel}>Pickup Point Fallback</Text><Text variant="labelSmall" style={s.switchSub}>Enable if delivery is weak in this area</Text></View><Switch value={isPickup} onValueChange={setIsPickup} color={theme.colors.primary}/></View>
          <Button mode="contained" onPress={handleSubmit(onSubmit)} loading={isPending} style={s.btn} contentStyle={{paddingVertical:SPACING.xs}}>Save Address</Button>
        </View>
