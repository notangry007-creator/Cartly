...
import {
  DELIVERY_TIME_SLOTS,
  SCHEDULED_DELIVERY_ZONES,
  getAvailableDates,
  formatDeliveryDate,
} from '../../src/data/delivery-slots';
import { formatNPR } from '../../src/utils/helpers';
import { DeliveryOption, OrderItem, PaymentMethod, ScheduledDeliverySlot } from '../../src/types';
...
const DELIVERY_ETA: Record<DeliveryOption, string> = {
...
export default function CheckoutScreen() {
  ...
  const [scheduledSlot, setScheduledSlot] = useState<ScheduledDeliverySlot | null>(null);
  const supportsScheduled = SCHEDULED_DELIVERY_ZONES.includes(zoneId as any);
  const availableDates = supportsScheduled ? getAvailableDates(7) : [];
  ...
  if (appliedCoupon) {
    discount = appliedCoupon.type === 'percent'
      ? Math.min(Math.round(subtotal * appliedCoupon.value / 100), appliedCoupon.maxDiscount ?? Infinity)
      : appliedCoupon.value;
  }
    ...
    try {
      ...
      if (payMethod === 'wallet') {
        ...
        await addTx({
          ...
          balance: user.walletBalance - total,
        });
      }
  ...
  return (
    <View style={[s.container, { paddingTop: insets.top }]}>
        ...
        {STEPS.map((st, i) => (
          <React.Fragment key={st}>
            <TouchableOpacity
              ...
              disabled={i >= step}
            >
              <View style={[s.stepCircle, i <= step && s.scActive, i < step && s.scDone]}>
                {i < step
                  ? <Ionicons name="checkmark" size={14} color="#fff" />
                  : <Text style={[s.sNum, i === step && s.sNumA]}>{i + 1}</Text>
                }
              </View>
              <Text variant="labelSmall" style={[s.sLabel, i === step && s.sLabelA]}>{st}</Text>
            </TouchableOpacity>
            {i < STEPS.length - 1 && (
              <View style={[s.stepLine, i < step && s.slDone]} />
            )}
          </React.Fragment>
        ))}
      </View>
      ...
      <ScrollView style={s.scroll} showsVerticalScrollIndicator={false}>
        ...
        {step === 0 && (
          <View style={s.stepContent}>
            ...
            {loadAddr ? (
              ...
              <View style={s.noAddr}>
                <Text variant="bodyMedium" style={{ color: '#666' }}>No saved addresses.</Text>
              ...
              addresses.map(addr => (
                <TouchableOpacity
                  key={addr.id}
                  onPress={() => setSelAddrId(addr.id)}
                  activeOpacity={0.8}
                >
                  <Surface
                    style={[
                      s.optCard,
                      (selAddrId === addr.id || (!selAddrId && addr.isDefault)) && s.optCardA,
                    ]}
                    elevation={1}
                  >
                    <RadioButton.Android
                      value={addr.id}
                      status={(selAddrId === addr.id || (!selAddrId && addr.isDefault)) ? 'checked' : 'unchecked'}
                      onPress={() => setSelAddrId(addr.id)}
                      color={theme.colors.primary}
                    />
                    <View style={s.addrInfo}>
                      <Text variant="labelMedium" style={s.addrLabel}>{addr.label}</Text>
                      <Text variant="bodySmall" style={s.addrTxt}>
                      ...
                      {addr.isPickupPointFallback && (
                        <View style={{ flexDirection: 'row', alignItems: 'center', gap: 3, marginTop: 4 }}>
                          ...
                          <Text style={{ color: '#FF8F00', fontSize: 11 }}>Pickup point fallback</Text>
                        </View>
                      )}
                    </View>
                  </Surface>
        ...
        {step === 1 && (
          <View style={s.stepContent}>
            ...
            {zone.deliveryOptions.map(opt => (
              <TouchableOpacity key={opt} onPress={() => setDOpt(opt)} activeOpacity={0.8}>
                <Surface style={[s.optCard, dOpt === opt && s.optCardA]} elevation={1}>
                  <RadioButton.Android
                    value={opt}
                    status={dOpt === opt ? 'checked' : 'unchecked'}
                    onPress={() => setDOpt(opt)}
                    color={theme.colors.primary}
                  />
                  <View style={s.optInfo}>
                    <Text variant="titleSmall" style={s.optLabel}>{DELIVERY_LABELS[opt]}</Text>
                    <Text variant="bodySmall" style={s.optEta}>{DELIVERY_ETA[opt]}</Text>
                  </View>
                  <Text variant="titleSmall" style={s.optFee}>
            ...
            {/* Scheduled delivery slot picker — only for supported zones */}
            {supportsScheduled && (
              <View style={{ marginTop: SPACING.md }}>
                <Text variant="titleSmall" style={s.stepTitle}>📅 Schedule Delivery (Optional)</Text>
                ...
                <ScrollView horizontal showsHorizontalScrollIndicator={false} style={{ marginBottom: SPACING.sm }}>
                  <View style={{ flexDirection: 'row', gap: SPACING.sm, paddingHorizontal: 2 }}>
                    <TouchableOpacity
                      style={[s.slotChip, !scheduledSlot && s.slotChipA]}
                      onPress={() => setScheduledSlot(null)}
                    >
                      <Text style={[s.slotChipTxt, !scheduledSlot && s.slotChipTxtA]}>ASAP</Text>
                    </TouchableOpacity>
                    {availableDates.map(date => (
                      <TouchableOpacity
                        key={date}
                        style={[s.slotChip, scheduledSlot?.date === date && s.slotChipA]}
                        onPress={() => setScheduledSlot(prev => ({
                          date,
                          slotId: prev?.slotId ?? DELIVERY_TIME_SLOTS[0].id,
                          slotLabel: prev?.slotLabel ?? DELIVERY_TIME_SLOTS[0].label,
                        }))}
                      >
                        <Text style={[s.slotChipTxt, scheduledSlot?.date === date && s.slotChipTxtA]}>
                ...
                {scheduledSlot && (
                  <View style={{ flexDirection: 'row', flexWrap: 'wrap', gap: SPACING.sm }}>
                    {DELIVERY_TIME_SLOTS.map(slot => (
                      <TouchableOpacity
                        key={slot.id}
                        style={[s.slotChip, scheduledSlot.slotId === slot.id && s.slotChipA]}
                        onPress={() => setScheduledSlot(prev => prev ? { ...prev, slotId: slot.id, slotLabel: slot.label } : null)}
                      >
                        <Text style={[s.slotChipTxt, scheduledSlot.slotId === slot.id && s.slotChipTxtA]}>
                          {slot.label}
                        </Text>
                      </TouchableOpacity>
                    ))}
                ...
                {scheduledSlot && (
                  <Text variant="labelSmall" style={{ color: '#2E7D32', marginTop: SPACING.xs }}>
                    ✓ Scheduled: {formatDeliveryDate(scheduledSlot.date)}, {scheduledSlot.slotLabel}
                  </Text>
                )}
              </View>
            )}
        ...
        {step === 2 && (
          <View style={s.stepContent}>
            ...
            {/* COD option */}
            {zone.codAvailable ? (
              <TouchableOpacity onPress={() => setPayMethod('cod')} activeOpacity={0.8}>
                <Surface style={[s.optCard, payMethod === 'cod' && s.optCardA]} elevation={1}>
                  <RadioButton.Android
                    value="cod"
                    status={payMethod === 'cod' ? 'checked' : 'unchecked'}
                  ...
                  <View style={s.optInfo}>
                    <View style={{ flexDirection: 'row', alignItems: 'center', gap: SPACING.xs }}>
                      ...
                      <Text variant="titleSmall" style={s.optLabel}>Cash on Delivery</Text>
                    </View>
                    <Text variant="bodySmall" style={s.optEta}>
              ...
              <Surface style={[s.optCard, { backgroundColor: '#f8f8f8' }]} elevation={0}>
                <View style={s.optInfo}>
                  <Text variant="titleSmall" style={{ color: '#bbb' }}>Cash on Delivery</Text>
                  <Text variant="bodySmall" style={{ color: '#ccc' }}>Not available in your zone</Text>
                </View>
              </Surface>
            )}

            {/* Wallet option */}
            <TouchableOpacity onPress={() => setPayMethod('wallet')} activeOpacity={0.8}>
              <Surface style={[s.optCard, payMethod === 'wallet' && s.optCardA]} elevation={1}>
                <RadioButton.Android
                  value="wallet"
                  status={payMethod === 'wallet' ? 'checked' : 'unchecked'}
                ...
                <View style={s.optInfo}>
                  <View style={{ flexDirection: 'row', alignItems: 'center', gap: SPACING.xs }}>
                    ...
                    <Text variant="titleSmall" style={s.optLabel}>Buy Wallet</Text>
                  </View>
                  <Text variant="bodySmall" style={s.optEta}>
                    Balance: {formatNPR(user?.walletBalance ?? 0)}
                    {(user?.walletBalance ?? 0) < total ? ' · Insufficient' : ''}
                  </Text>
                </View>
              </Surface>
            </TouchableOpacity>
          ...
          <View style={s.stepContent}>
            ...
            <Surface style={s.revSec} elevation={1}>
              <View style={{ flexDirection: 'row', alignItems: 'center', gap: SPACING.xs, marginBottom: SPACING.xs }}>
                ...
                <Text variant="titleSmall" style={{ fontWeight: '700', color: '#222' }}>Delivery To</Text>
              </View>
              {selAddr && (
                <Text variant="bodySmall" style={{ color: '#555' }}>
                  {selAddr.label}: {selAddr.landmark}, Ward {selAddr.ward}, {selAddr.municipality}
                </Text>
              )}
            </Surface>
            ...
            {/* Items */}
            <Surface style={s.revSec} elevation={1}>
              <Text variant="titleSmall" style={{ fontWeight: '700', color: '#222', marginBottom: SPACING.sm }}>
                Items ({resolved.length})
              </Text>
              {resolved.map(({ item, p, v }) => (
                <View key={item.productId + item.variantId} style={{ paddingVertical: 3 }}>
                  <Text variant="bodySmall" style={{ color: '#333', fontWeight: '500' }} numberOfLines={1}>
                    {p?.title}
                  </Text>
                  <Text variant="bodySmall" style={{ color: '#888' }}>
                    {v?.label} x{item.quantity} = {formatNPR((v?.price ?? 0) * item.quantity)}
                  </Text>
                </View>
              ))}
            </Surface>
            ...
            <Surface style={s.revSec} elevation={1}>
              ...
              {[
                ...
                ['Shipping', formatNPR(shippingFee)],
                codFee > 0 ? ['COD Fee', formatNPR(codFee)] : null,
                discount > 0 ? ['Discount', '- ' + formatNPR(discount)] : null,
              ].filter(Boolean).map(([l, v]) => (
                <View key={l as string} style={{ flexDirection: 'row', justifyContent: 'space-between', marginBottom: 4 }}>
                  <Text variant="bodySmall" style={{ color: '#555' }}>{l}</Text>
                  <Text variant="bodySmall" style={{ color: l === 'Discount' ? '#2E7D32' : '#333' }}>{v}</Text>
                </View>
              ))}
              ...
              <View style={{ flexDirection: 'row', justifyContent: 'space-between', marginTop: 4 }}>
                <Text variant="bodySmall" style={{ color: '#555' }}>Payment</Text>
                <Text variant="bodySmall">
                  {payMethod === 'cod' ? 'Cash on Delivery' : 'Buy Wallet'}
                </Text>
              </View>
            </Surface>
          </View>
      ...
      <View style={[s.bottomBar, { paddingBottom: insets.bottom + SPACING.sm }]}>
        {step > 0 && (
          <Button mode="outlined" onPress={goBack}>Back</Button>
        )}
        {step < STEPS.length - 1 ? (
          <Button
            mode="contained"
...
// Slot chip styles added for scheduled delivery
const s = StyleSheet.create({
  ...
  slotChipA: { borderColor: theme.colors.primary, backgroundColor: theme.colors.primaryContainer },
  slotChipTxt: { color: '#555', fontWeight: '600', fontSize: 13 },
  slotChipTxtA: { color: theme.colors.primary },
});
